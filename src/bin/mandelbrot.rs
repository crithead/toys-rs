// Mandelbrot set generation from Programming Rust
// Sample parameters
...
/// on the origin.  If 'c' seems to be amember (more precicely, if we reached
/// the iteration limit without being able to prove that 'c' is not a member),
/// return 'None'.
...
///
/// Specifically, 's' should have the for <left><sep><right>, where <sep> is
/// the character given by the 'separator' argument, the <left> and <right>
/// are both strings thgat can be parsed by 'T::from_str'.
///
...
/// Give the row and column of a pixel in the output image, return the
/// corresponding point on the complex plane.
///
/// 'bounds' is a pair giving the width tand height off the image in pixels.
...
fn pixel_to_point(
    ...
    Complex {
        ...
        // Why subtract here? pixel.1 increases as we go down,
...
/// which holds one greyscale pixel per byte.  The 'upper_left' and
